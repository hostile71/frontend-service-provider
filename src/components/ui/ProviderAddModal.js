/**
 * Provider Add Modal Component
 * 
 * Form for adding new provider with business information
 */
...
const ProviderAddModal = ({ isOpen, onClose, onSuccess }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    type: 'provider', // Auto-selected for provider
    status: 'active',
    role_id: '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (error.errors) {
        ...
        setErrors({ general: 'An error occurred while creating the provider' });
      }
    }
  };
